...
class StatsCollection {
  ...
  async init() {
    ...
    this.origin = 'aedge-dashboard-satellite';
    ...
    this.connection.addService(constants.SERVICE('aedge').walletUi);
  ...
  async updateRunningServices() {
    ...
    await Promise.all(this.enabledServices.filter(service => service !== plotterService).map(async service => {
      let isRunning = await this.daemonApiClient.isServiceRunning(constants.SERVICE('aedge')[service]);
